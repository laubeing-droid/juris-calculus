...
DIST = f"juris_calculus-{__version__}.dist-info"
...
def _wheel(tmp_path: Path, mutation: str = "valid") -> Path:
    ...
    wheel = tmp_path / f"juris_calculus-{__version__}-py3-none-any.whl"
...
def test_payload_is_derived_from_the_manual_production_classes() -> None:
    ...
    # + 3 v4.2 legal-candidate assets (fact_extraction.py,
    # applicable_law.py, configs/deadline_rules_cn.v1.yaml).
    assert len(payload) == 59
    for required in (
        "compiler_core/procedure.py",
        "compiler_core/domain_composition.py",
        "compiler_core/assurance.py",
    ...
    assert "schemas/jc-v5.schema.json" in payload
...
def test_exact_synthetic_wheel_is_accepted(tmp_path: Path) -> None:
    ...
    assert report["entry_count"] == 65  # 59 payload + 6 dist-info entries
