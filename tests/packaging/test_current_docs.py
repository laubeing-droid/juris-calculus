...
def test_historical_guides_are_not_current_authority() -> None:
    ...
    assert "Historical replay (not current authority)" in docs_index
    assert "V3_HISTORICAL_REPLAY.md" in docs_index
...
def test_release_docs_do_not_claim_external_promotion_is_complete() -> None:
    ...
    assert "production Ed25519" in handoff
