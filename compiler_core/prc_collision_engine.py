#!/usr/bin/env python3
"""PRC Collision Engine — bounded CBL+SPC candidate experiment.
...
class PRCCollisionEngine:
    ...
    def rule_inventory(self) -> Dict[str, Dict]:
        """报告 CBL+SPC 两条实验轨道的语料边界。

        阻断规则不经过通用 YAML LegalRule 加载器，当前均属于人工维护的
        PRC 边界规则；SPC 依据加载器写入的 CANDIDATE_ONLY 标记计数。
        """
