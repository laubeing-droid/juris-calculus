...
- A linked worktree stores `.git` as a pointer file, not a directory. Any isolated Git integration test must create an independent repository or exclude both `.git` forms; copying the pointer can advance the real branch. W0-02 now gates the AUTO receipt path with an independent-repository test whose command creates a non-empty commit and proves the receipt binds its exact path and bytes.
