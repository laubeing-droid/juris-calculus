...
- The remaining separately-evaluated medium-risk candidates are `domain_mapping.json` (cross-domain test), `juris_blueprint.json` (US lookup + blind reconstruction tooling + config path helper), `lexicon_index.json` (distill tool), and `perf_patterns.yaml` (performance baseline/blueprint tools and tests).
