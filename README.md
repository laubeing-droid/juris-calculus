...
juris-calculus 是一个确定性的符号法律推理内核——不是一个法律应用，而是一套可嵌入法律 AI 产品的推理引擎。它把成文法编译成可执行的规则，用形式化验证的流水线推理，给每个结论挂上可审计的信任标签。当前版本 5.0.2（公共协议 jc/5.0）。
...
- **合同面**：124 个封闭合同类型（含 V5 对象组、本机记录背书 `LocalRecordV4` 与 18 个 `jc-business-root/1` 业务类型），`schemas/jc-v5.schema.json` 由代码确定性生成、禁止手改；MCP 侧是四工具 stdio 服务（`mcp_manifest.json`）。CLI、Python（`JCClient`）与 MCP 三个入口共用同一个 application service，语义完全一致。
...
- **业务能力（jc-business-root/1）**：有限情景条件本金计算经 `business_tasks_v1` 进入唯一正式主链——独立 checker 逐情景守恒验证、E[C]/E[U]/E[R] 分离、合法和解格、verify-only 双文件交付核验（核验零求值、不改密封审计包）；全部输出是条件模型分析，`formal_evidence` 如实为 kernel 待证。契约见 [docs/contracts/BUSINESS_ROOT.md](docs/contracts/BUSINESS_ROOT.md)。
