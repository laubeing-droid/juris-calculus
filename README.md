...
juris-calculus 是一个确定性的符号法律推理内核——不是一个法律应用，而是一套可嵌入法律 AI 产品的推理引擎。它把成文法编译成可执行的规则，用形式化验证的流水线推理，给每个结论挂上可审计的信任标签。当前版本 5.0.0（公共协议 jc/5.0）。
