...
| Python 测试 | 整改前已核验基线：347 passed, 38 skipped |
| 真实 stdio 门禁 | 整改后准入必须通过；上述基线不覆盖此门禁 |
...
`347 passed, 38 skipped` 是整改前已核验基线。整改后准入还必须通过真实 MCP 客户端/服务端 stdio 交互；仅执行 `python mcp_server.py --test` 不足以通过该门禁。
